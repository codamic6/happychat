      ...
      <aside className="w-80 border-r border-white/5 flex flex-col bg-[#0d0d0d]">
        <div className="p-6 space-y-6">
          <div className="flex items-center justify-between">
            <h2 className="text-xl font-bold font-headline text-white uppercase tracking-tighter">
          ...
          <div className="px-3 pb-6 space-y-1">
              ...
              >
                <div className="relative shrink-0">
                  <Avatar className="w-12 h-12 border border-white/10">
                    <AvatarImage src={`/api/avatar/${contact.id}`} />
                ...
                <div className="flex-1 text-left min-w-0">
                  ...
                  <p className="text-[10px] text-muted-foreground truncate font-bold uppercase tracking-widest">@{contact.username}</p>
                </div>
              </button>
            ))}
            {contactProfiles.length === 0 && (
              <div className="p-12 text-center space-y-4">
                ...
                <p className="text-xs text-muted-foreground italic">Your contact list is empty.</p>
                <Button 
                  variant="outline" 
                  size="sm" 
                  onClick={() => setIsAddContactOpen(true)}
                  className="border-primary/20 text-primary hover:bg-primary/10 rounded-full text-[10px] font-bold uppercase tracking-widest"
                >
                  Find Contacts
                </Button>
              </div>
      ...
      <div className="flex-1 flex flex-col bg-[#050505]">
        {selectedUser ? (
          <>
            ...
            >
              <div className="flex items-center gap-4">
                <Avatar className="w-10 h-10 border border-primary/20">
                  <AvatarImage src={`/api/avatar/${selectedUser.id}`} />
            ...
            <ScrollArea className="flex-1 p-6 relative">
              <div className="max-w-4xl mx-auto space-y-8 pb-12 flex flex-col items-center justify-center h-full opacity-30">
                ...
                <p className="text-sm font-bold uppercase tracking-widest italic">Secure conversation with {selectedUser.fullName.split(' ')[0]}</p>
              </div>
            </ScrollArea>
          ...
          <div className="flex-1 flex flex-col items-center justify-center space-y-6 opacity-20">
             ...
             <p className="text-sm font-bold uppercase tracking-[0.5em] italic">Select a conversation</p>
          </div>
        )}
      </div>
            ...
            <div className="p-6 h-full flex flex-col">
              ...
              <div className="flex flex-col items-center text-center space-y-6">
                ...
                <div className="space-y-1">
                  <h2 className="text-2xl font-bold font-headline tracking-tighter uppercase">{selectedUser.fullName}</h2>
                  <p className="text-primary text-[10px] font-bold uppercase tracking-[0.2em]">Active Now</p>
                </div>
                ...
                <Card className="w-full bg-white/5 border-white/5 p-4 space-y-4">
                  <div className="flex items-center gap-4 text-left">
                    ...
                    <div>
                      ...
                      <p className="text-xs text-white">{selectedUser.about || "Digital creator on HappyChat."}</p>
                    </div>
                  </div>
                  <div className="flex items-center gap-4 text-left">
...
function AddContactDialogContent({ onSuccess, currentUserId }: { onSuccess: () => void, currentUserId?: string }) {
  ...
  const handleAddContact = async () => {
    ...
    setIsAdding(true);
    try {
      // Create relationship (Mutual connection)
      ...
      toast({
        title: "Contact Added",
        description: `${foundUser.fullName} has been added.`
      });
      onSuccess();
    } catch (err) {
      toast({
  ...
  return (
    <DialogContent className="sm:max-w-md bg-[#0a0a0a] border-white/5 text-white p-0 overflow-hidden rounded-[2.5rem]">
      <DialogHeader className="p-8 pb-4">
        <DialogTitle className="text-2xl font-bold font-headline uppercase tracking-tight flex items-center gap-3">
        ...
        <AnimatePresence mode="wait">
          {foundUser ? (
            ...
            <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="text-center py-8 space-y-3">
              ...
              <p className="text-sm font-bold text-white uppercase">No user found</p>
            </motion.div>
          )}
        </AnimatePresence>
      </div>
